          ...
          .then(res => {
           // option1.title.text = res.data.title;
            ...
            if(res.data.length>0){
              option1.title.text = moment(this.sDateStr).format('YYYY年MM月DD日HH时') +'至'+moment(this.eDateStr).format('YYYY年MM月DD日HH时')+'多源融合实况检验——'+this.eleChineseName;
              option1.yAxis.name=this.eleChineseName;
              try{
                let idSz=this.stationIDs.split(',');
                for(let i=0;i<idSz.length;i++){
                  let seriels={};
                  seriels.name=idSz[i];
                  seriels.type='line';
                  seriels.showSymbol=true;
                  seriels.hoverAnimation=true;
                  let arr1=res.data.filter((v) => { return v.stationIdC===idSz[i] })
                  let arrData=[[arr1[0].datetime,arr1[0].czvalue]];
                  for(let j=1;j<arr1.length;j++){
                   // arrData.push(new Date(arr1[j].datetime.replace(/-/g,'/')),arr1[j].czvalue)
                    arrData.push([arr1[j].datetime,arr1[j].czvalue])
                  }
